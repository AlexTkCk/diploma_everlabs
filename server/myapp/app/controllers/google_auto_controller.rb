class GoogleAutoController < ApplicationController
  ...
  def reg
    ...
    # Проверяем наличие электронной почты в объекте аутентификации
    email = auth.info.email
    uid = auth.uid

    if email.blank?
      # В случае отсутствия электронной почты возвращаем ошибку
      return render json: { error: 'Email not found in authentication data' }, status: :unprocessable_entity
    end
    ...
    if user
      ...
      if user.save
        ...
        if new_room.save
          ...
          render json: { error: 'An error occurred while creating the room' }, status: :unprocessable_entity
        end
      else
