...
const CustomLink: React.FC<CustomLinkProps> = ({ icon: Icon, text, link }) => {
  ...
  return (
    ...
    >
      ...
      <Icon className={`h-16 shrink-0 w-16 top-1/2 left-1/2 p-2 ${themeConfig.accent} border-black border-2 rounded-full group-hover:rotate-[360deg] group-hover:left-[-0.7rem] transition-all easy-in-out duration-500`} />
    </RouterLink>
  );
};
