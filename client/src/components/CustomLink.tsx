...
const CustomLink: React.FC<CustomLinkProps> = ({ icon: Icon, text, link }) => {
  return (
    ...
    >
      ...
      <p className="absolute font-primary text-xl w-fit text-nowrap top-1/2 left-1/2 opacity-0 transform  -translate-y-1/2 border-2 border-black p-2 pl-4  group-hover:translate-x-2 group-hover:opacity-100 transition-all duration-200 delay-200 easy-in-out">
        {text}
      </p>
    </RouterLink>
  );
